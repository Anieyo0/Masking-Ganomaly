...
class Ganomaly(BaseModel):
    ...
    def forward_g(self):
        ...
        if self.opt.use_context_pred:
            ...
            self.fake, self.latent_i, self.latent_o = self.netg(self.input, masked_input=self.masked_input)
        else:
    ...
    def backward_g(self):
        ...
        # Encoder Loss 구성 요소
        # 1. 원본 이미지와 마스킹된 이미지 사이의 latent 차이
        enc_loss_masked = self.l_enc(self.latent_o, self.latent_i)
